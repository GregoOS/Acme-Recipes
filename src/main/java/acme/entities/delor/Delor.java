...
public class Delor extends AbstractEntity {
	...
	@NotBlank
	@Column(unique = true)
	@Pattern(regexp="^\\d{6}:\\d{6}$", message = "{delor.regex.code}") //copied from item   \\d{6}:\\d{6}
...
//	objetivo -> ^\\w{6}:mmyy:dd$
	...
	@NotBlank
	@Length(max=255)
	private String description; //explanation
	
	@NotNull
	@Temporal(TemporalType.TIMESTAMP)
	private Date startDate; //these 2 are the period
	...
	@NotNull
	@Valid
	private Money budget; //income
	
	@URL
	private String link; //more-info
